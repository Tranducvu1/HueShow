...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($slug) || empty($description)) {
        ...
        if (createEvent($title, $slug, $description, $image, $event_date, $status, $featured)) {
...
<head>
    <meta charset="UTF-8">
    <title>Thêm sự kiện - Hueshow Admin</title>
    ...
    <style>
        /* Dùng lại style như articles, có thể thêm chút tùy chỉnh */
...
<div class="admin-wrapper">
    ...
    <div class="main-content">
        <div class="form-container">
            <h2 style="margin-bottom:20px;">➕ Thêm sự kiện mới</h2>
            <?php if($error): ?><div class="error"><?= htmlspecialchars($error) ?></div><?php endif; ?>
            <?php if($success): ?><div class="success"><?= htmlspecialchars($success) ?> <a href="../admin_events.php">Quay lại danh sách</a></div><?php endif; ?>
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="event_date" value="<?= htmlspecialchars($_POST['event_date'] ?? '') ?>">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label><input type="checkbox" name="featured" value="1" <?= (isset($_POST['featured']) && $_POST['featured']) ? 'checked' : '' ?>> Sự kiện nổi bật</label>
                </div>
                <div class="form-group">
                    <button type="submit"><i class="fas fa-plus"></i> Thêm sự kiện</button>
