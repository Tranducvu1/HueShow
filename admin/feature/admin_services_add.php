...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($description)) {
        ...
        if (createService($title, $description, $image, $event_date, $status, $featured)) {
            $success = 'Thêm dịch vụ thành công!';
            $_POST = [];
        } else {
            $error = 'Có lỗi xảy ra khi lưu vào database: ' . $conn->error;
        }
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Thêm dịch vụ - Hueshow Admin</title>
...
<div class="admin-wrapper">
    ...
    <div class="main-content">
        <div class="form-container">
            <h2 style="margin-bottom:20px;">➕ Thêm dịch vụ mới</h2>
            <?php if($error): ?><div class="error"><?= htmlspecialchars($error) ?></div><?php endif; ?>
            <?php if($success): ?><div class="success"><?= htmlspecialchars($success) ?> <a href="../admin_services.php">Quay lại danh sách</a></div><?php endif; ?>
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label><input type="checkbox" name="featured" value="1" <?= (isset($_POST['featured']) && $_POST['featured']) ? 'checked' : '' ?>> Dịch vụ nổi bật</label>
                </div>
                <div class="form-group">
                    <button type="submit"><i class="fas fa-plus"></i> Thêm dịch vụ</button>
