...
// Đường dẫn tuyệt đối đến thư mục uploads/banners (tính từ file này)
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExt, $allowed)) {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $destination)) {
                // Lưu đường dẫn tương đối tính từ thư mục gốc
                $imagePath = 'uploads/banners/' . $newFileName;
            } else $error = 'Không thể upload ảnh.';
        } else $error = 'Chỉ chấp nhận file ảnh.';
    } else $error = 'Vui lòng chọn ảnh banner.';
    
    if (empty($error) && !empty($title) && !empty($imagePath)) {
        if (createBanner($title, $imagePath, $link, $order_index, $status)) {
            $success = 'Thêm banner thành công!';
            $_POST = [];
        } else $error = 'Lỗi lưu database.';
        ...
        <div class="card">
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    <label>Tiêu đề *</label>
                    <input type="text" name="title" value="<?= htmlspecialchars($_POST['title'] ?? '') ?>" placeholder="VD: Khuyến mãi mùa hè" required>
                </div>
                <div class="form-group">
                    ...
                    <img id="preview" class="preview-img" alt="Xem trước ảnh">
                </div>
                <div class="form-group">
                    <label>Link (URL đích)</label>
                    <input type="url" name="link" value="<?= htmlspecialchars($_POST['link'] ?? '') ?>" placeholder="https://example.com/sale">
                    <small>Để trống nếu banner chỉ mang tính trang trí</small>
                </div>
                <div class="form-group">
                    <label>Thứ tự hiển thị</label>
                    <input type="number" name="order_index" value="<?= htmlspecialchars($_POST['order_index'] ?? '0') ?>" step="1">
                </div>
                <div class="form-group">
                    <label>Trạng thái</label>
                    <select name="status">
                        <option value="active" <?= (isset($_POST['status']) && $_POST['status'] == 'active') ? 'selected' : '' ?>>Kích hoạt</option>
                        <option value="inactive" <?= (isset($_POST['status']) && $_POST['status'] == 'inactive') ? 'selected' : '' ?>>Ẩn</option>
                    </select>
                </div>
                <div class="form-group">
